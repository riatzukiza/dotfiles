...
class BracketMatcher {
    ...
    private skipComment(text: string, i: number): number {
        let j = i + 1;
                        ...
                        if (stack.length > 0) {
                            ...
                            if (lastOpen.open === pair.open && lastOpen.close === pair.close) {
                                ...
                                const openRange = new vscode.Range(
                                    lastOpen.position,
                                    document.positionAt(lastOpen.startIndex + lastOpen.open.length)
                                );
                                const closeRange = new vscode.Range(
                                    position,
                                    document.positionAt(absoluteIndex + pair.close.length)
                                );
...
export function activate(context: vscode.ExtensionContext) {
    ...
    const textChangeDisposable = vscode.workspace.onDidChangeTextDocument((event: vscode.TextDocumentChangeEvent) => {
        const editor = vscode.window.activeTextEditor;
